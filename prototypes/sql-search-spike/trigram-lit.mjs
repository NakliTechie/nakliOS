// SPIKE COPY of sys/rig/fileops/trigram.mjs, with ONE change: a TRI node also
// carries `run` (the folded literal), so the plan can be rendered as an FTS5 MATCH
// expression. Planner logic is otherwise byte-identical, so the SQL spike and the
// shipped index are compared on the SAME hardened plan — only the postings store differs.
...
function triOf(literal) {
  ...
  return { op: 'TRI', tris: trigrams(folded), run: folded };
}
